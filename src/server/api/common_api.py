    ...
    with engine.connect() as connection:
        ...
        if len(rows) > 0:
            for row in rows:               
                ...
                person_details = safe_requests.get(person_url, headers={"x-api-key": SHELTERLUV_SECRET_TOKEN}, timeout=60).json()
                if "ID" in person_details:
                    ...
                    for animal_id in animal_ids:
                        ...
                        animal_details = safe_requests.get(animal_url, headers={"x-api-key": SHELTERLUV_SECRET_TOKEN}, timeout=60).json()
...
def get_person_animal_events(matching_id, animal_id):
    ...
    with engine.connect() as connection:
        ...
        if len(rows) > 0:
            ...
            event_details = safe_requests.get(animal_url, headers={"x-api-key": SHELTERLUV_SECRET_TOKEN}, timeout=60).json()
